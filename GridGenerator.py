...
def GridGenerator(self,
    ...
    """
    with torch.no_grad():
        #  No need to track gradients when formulating 
        #  the grid distances
        ...
        transformation_matrix[1][2] = c * ((torch.cos(alpha) - (torch.cos(beta)*torch.cos(gamma)))\
                                            / torch.sin(gamma))
    ...
    points = torch.matmtul(transformation_matrix, point_coordinates[:,1:])
